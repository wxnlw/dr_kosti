(function birthdayPlovGame() {
  ...
  function getItemLabel(itemId) {
    ...
    for (var i = 0; i < all.length; i += 1) {
      if (all[i].id === itemId) return all[i].label;
    }
    return itemId;
  }
  ...
  function renderInventory() {
    ...
    foodItems.forEach(function (item) {
      ...
      sprite.style.backgroundImage = makePixelSprite(item.id);
  ...
  function renderPrepTray() {
    ...
    ["rice", "carrot", "onion", "garlic"].forEach(function (itemId) {
      if (!state.prepped[itemId]) return;
      var button = document.createElement("button");
      button.type = "button";
      button.className = "tray-item";
      button.dataset.itemId = itemId;
      var sprite = document.createElement("div");
      sprite.className = "sprite sprite--tray";
      sprite.style.backgroundImage = makePixelSprite(itemId);
      var label = document.createElement("div");
      label.className = "tray-item__label";
      label.textContent = getItemLabel(itemId);
      button.appendChild(sprite);
      button.appendChild(label);
      button.addEventListener("click", onTrayItemClick);
      refs.prepTray.appendChild(button);
    });
  }
